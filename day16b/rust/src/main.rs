...
fn main() {
    ...
    let lowest_score_paths = finished_reindeers
        .iter()
        .filter(|reindeer| reindeer.score == lowest_score)
        .fold(HashSet::new(), |acc, reindeer| {
            let mut new_acc = acc.clone();
            new_acc.extend(reindeer.path.clone());
            new_acc
        })
        .len()
        + 1;

    println!("Lowest score paths: {}", lowest_score_paths);
}
